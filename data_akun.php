...
// Auto-seed Website account if not present
try {
    $checkWeb = $pdo->prepare("SELECT COUNT(*) FROM akun_sosmed WHERE UPPER(platform) = 'WEBSITE'");
    $checkWeb->execute();
    if ((int)$checkWeb->fetchColumn() === 0) {
        $seedWeb = $pdo->prepare("INSERT INTO akun_sosmed (platform, nama_akun, jumlah_follower) VALUES ('WEBSITE', '@https://probolinggokab.go.id/', 0)");
        $seedWeb->execute();
    }
} catch (PDOException $e) {
    // Ignore error if table structure varies slightly
}
        ...
        >
            ...
            <div class="content-filter-bar">
                ...
                >
                    ...
                    <button
                        type="button"
                        class="filter-tab account-filter-tab filter-tab-website"
                        data-account-filter="website"
                    >
